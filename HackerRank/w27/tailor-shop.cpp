...
int minButtons(int n, int p, vector<int> &v) {
  ...
  for(int i = 0; i < n; i++) {
    ...
    while(buttonsGroupSizes.find(numButtonsInGroup) != buttonsGroupSizes.end()) {
      // handle set size already exists
      numButtonsInGroup++;
    }
    
    buttonsGroupSizes.insert(numButtonsInGroup);
